...
def perform_search(query, video_ids=None):
    """Search through videos - same logic as original"""
    if not query:
        return []
...
def process_search_job(job_id):
    """Process a search job in background"""
    try:
...
@app.route('/search/<job_id>', methods=['GET'])
...
def list_jobs():
    ...
    jobs_list = list(search_jobs.values())
    
    if user_id:
        jobs_list = [j for j in jobs_list if str(j.get('user_id')) == str(user_id)]
...
if __name__ == '__main__':
    ...
    print(f"🚀 Search Service starting on port {port}")
