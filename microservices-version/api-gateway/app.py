...
def videos(video_id=None):
    ...
    try:
        if video_id:
            url = f"{VIDEO_SERVICE_URL}/api/videos/{video_id}"
            ...
            url = f"{VIDEO_SERVICE_URL}/api/videos"
            if request.method == 'GET':
...
def user_videos(user_id):
    ...
    try:
        if request.method == 'GET':
            resp = requests.get(f"{VIDEO_SERVICE_URL}/api/users/{user_id}/videos", params=request.args, headers=forward_headers())
        else:  # POST
            resp = requests.post(f"{VIDEO_SERVICE_URL}/api/users/{user_id}/videos", json=request.json, headers=forward_headers())
        return jsonify(resp.json()), resp.status_code
    except requests.exceptions.RequestException:
        return jsonify({"error": "Video service unavailable"}), 503
